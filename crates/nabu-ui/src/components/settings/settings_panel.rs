...
// Must match AppSettings in src-tauri/src/settings.rs
...
pub fn SettingsPanel() -> impl IntoView {
    ...
    let save_settings = move |updated_settings: AppSettings| {
        spawn_local(async move {
    ...
    let (active_tab, set_active_tab) = signal("General & Modules");
        ...
        <div class="settings-panel flex h-full">
            <div class="tabs w-1/4 border-r border-gray-700 bg-gray-900 p-4">
                {tabs.iter().map(|tab| {
                    ...
                    view! {
                        <button
                            class=move || format!("block w-full text-left p-2 {}", if active_tab.get() == tab { "bg-gray-800 text-white" } else { "text-gray-400" })
            ...
            <div class="content w-3/4 p-4 text-white">
                {move || match active_tab.get().as_str() {
                    "General & Modules" => view! { <GeneralSettings settings=settings save=save_settings.clone() /> }.into_any(),
                    "Editor & Notion Block Menu" => view! { <EditorSettings settings=settings save=save_settings.clone() /> }.into_any(),
                    "Whispr AI & Voice Dictation" => view! { <WhisprSettings settings=settings save=save_settings.clone() /> }.into_any(),
                    "Appearance & Opacity Controls" => view! { <AppearanceSettings settings=settings save=save_settings.clone() /> }.into_any(),
                    "Files, Trash & Sandboxing" => view! { <FileSettings settings=settings save=save_settings.clone() /> }.into_any(),
                    "Folder Graph & Canvas" => view! { <GraphSettings settings=settings save=save_settings.clone() /> }.into_any(),
                    _ => view! {}.into_any(),
                }}
            </div>
        </div>
...
#[component]
fn GeneralSettings(settings: RwSignal<AppSettings>, save: impl Fn(AppSettings) + 'static + Clone) -> impl IntoView {
    view! {
...
#[component]
fn EditorSettings(settings: RwSignal<AppSettings>, save: impl Fn(AppSettings) + 'static + Clone) -> impl IntoView {
    view! {
        <h2 class="text-xl font-bold mb-4">"Editor & Notion Block Menu"</h2>
        <div class="space-y-4">
            <label class="block">"Editing Mode: "
            <select class="bg-gray-800 p-1" value=move || settings.get().editor_mode on:change=move |ev| {
                let mut s = settings.get();
...
#[component]
fn WhisprSettings(settings: RwSignal<AppSettings>, save: impl Fn(AppSettings) + 'static + Clone) -> impl IntoView {
    view! {
        <h2 class="text-xl font-bold mb-4">"Whispr AI & Voice Dictation"</h2>
        <div class="space-y-4">
            <label class="block">"Model: "
            <select class="bg-gray-800 p-1" value=move || settings.get().whisper_model on:change=move |ev| {
                let mut s = settings.get();
...
#[component]
fn AppearanceSettings(settings: RwSignal<AppSettings>, save: impl Fn(AppSettings) + 'static + Clone) -> impl IntoView {
    view! {
        <h2 class="text-xl font-bold mb-4">"Appearance & Opacity Controls"</h2>
        <div class="space-y-4">
            <label class="block">"Theme: "
            <select class="bg-gray-800 p-1" value=move || settings.get().theme on:change=move |ev| {
                let mut s = settings.get();
...
#[component]
fn FileSettings(settings: RwSignal<AppSettings>, save: impl Fn(AppSettings) + 'static + Clone) -> impl IntoView {
    view! {
        <h2 class="text-xl font-bold mb-4">"Files, Trash & Sandboxing"</h2>
        <div class="space-y-4">
            <label class="block">"Default New Note Path: "
            <select class="bg-gray-800 p-1" value=move || settings.get().default_new_note_path on:change=move |ev| {
                let mut s = settings.get();
            ...
            <select class="bg-gray-800 p-1" value=move || settings.get().trash_retention_policy on:change=move |ev| {
                let mut s = settings.get();
...
#[component]
fn GraphSettings(settings: RwSignal<AppSettings>, save: impl Fn(AppSettings) + 'static + Clone) -> impl IntoView {
    view! {
        ...
        <div class="space-y-4">
            ...
            <select class="bg-gray-800 p-1" value=move || settings.get().folder_click_behavior on:change=move |ev| {
                let mut s = settings.get();
