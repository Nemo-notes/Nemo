...
| Sandboxed HTML | `src/renderer/src/shared/components/SandboxedHtml.tsx` | Working | Low | Secure HTML rendering component. **Note:** `readNote` and `search` API methods now implemented via IPC (Phase 6.2). |
...
| Pane Layout | `src/renderer/src/features/vault/PaneLayout.tsx` | Working | Medium | Split-pane layout management. **Note:** Per-tab content rendering now implemented via `NoteViewForTab` (Phase 6.2). |
...
| note.create command | `src/renderer/src/shared/commands/registry.ts` | Working | Medium | Command Palette "Create new note" now implemented with interactive flow via `vaultCommands.createNote` (Phase 6.2). |
...
All incomplete features from Phase 6.1 have been repaired in Phase 6.2:

1. **Sandboxed HTML `readNote`/`search` methods** - Now implemented via IPC (Phase 6.2)
2. **`note.create` command** - Now implemented with interactive flow (Phase 6.2)
3. **PaneLayout per-tab rendering** - Now implemented via `NoteViewForTab` (Phase 6.2)

All features are now fully implemented and functional. No architectural changes are required.