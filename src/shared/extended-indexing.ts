 ...
 */
function addTokenPosition(index: ExtendedSearchIndex, token: string, filePath: string, line: number): void {
 ...
 */
function addNamespacedParentTags(tagIndex: Map<string, Set<string>>, tag: string, filePath: string): void {
 ...
 */
function extractBlockRefs(index: ExtendedSearchIndex, text: string, filePath: string, line: number | undefined): void {
...
function parseYamlFields(yaml: string): {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (key === 'tags') {
      ...
      if (inlineMatch) {
        const items = inlineMatch[1].split(',').map(t => t.trim()).filter(t => t.length > 0);
        tags.push(...items);
      ...
      if (afterColon.length > 0 && afterColon.startsWith('[')) {
        ...
        const items = afterColon.slice(1, -1).split(',').map(t => t.trim()).filter(t => t.length > 0);
