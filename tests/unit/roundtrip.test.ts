...
describe('Round-trip fidelity (Req 10.4)', () => {
  for (const filename of fixtureFiles) {
    it(`round-trips: ${filename}`, async () => {
      ...
      expect(
        ast1.type,
        `[${filename}] AST root must have type 'root'`,
      ).toBe('root');
      ...
      expect(
        typeof serialized,
        `[${filename}] serializeAST must return a string`,
      ).toBe('string');
      ...
      expect(
        norm2,
        `[${filename}] normalised AST after round-trip must match original`,
      ).toEqual(norm1);
      ...
      if (yaml1 !== null) {
        ...
        expect(
          obj2,
          `[${filename}] YAML front matter keys/values must be preserved`,
        ).toEqual(obj1);
      } else {
